...
/// Sends this computer's GCController as `GBG1` to a remote GBear host.
final class GBearGuestGamepadSender: @unchecked Sendable {
    ...
    private func bindAll() {
        for controller in GCController.controllers() {
            ...
            pad.valueChangedHandler = { [weak self] gamepad, _ in
                ...
                self.send(GBearHostLocalGamepad.snapshot(gamepad, seat: Int(self.joinSeat)))
            }
        }
    }

    private func send(_ event: GBearGamepadEventFormat.Event) {
        let packet = GBearGamepadEventFormat.pack(
            seat: event.seat,
