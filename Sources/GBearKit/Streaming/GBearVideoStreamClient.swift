...
final class GBearVideoStreamClient: @unchecked Sendable {
    ...
    private let networkQueue = DispatchQueue(label: "GBearGuest.video.network", qos: .userInitiated)
    private let decodeQueue = DispatchQueue(label: "GBearGuest.video.decode", qos: .userInitiated)
    ...
    private func receiveHeader() {
        ...
        connection.receive(minimumIncompleteLength: 13, maximumLength: 13) { [weak self] data, _, isComplete, error in
            ...
            guard magic == GBearStreamPorts.videoMagic else {
                self.receiveHeader()
                return
            }
