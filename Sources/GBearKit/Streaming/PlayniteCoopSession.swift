...
enum PlayniteCoopClientKind: String, Sendable, Codable, Equatable {
    /// This Mac is playing locally (no video client; feeds a virtual pad from GCController).
    case localHost
    /// Phone companion app (receives video, sends PNG1).
    case companion
...
/// One player slot (1…8). `joinSeat` is the PNG1 tag frozen at join; `seat` is the current virtual pad.
struct PlayniteCoopSeat: Sendable, Equatable, Identifiable {
    ...
    let seat: Int
    /// Seat written into PNG1 by this client. Host remaps joinSeat → seat.
    let joinSeat: Int
    let kind: PlayniteCoopClientKind
    let joinedAt: Date
    ...
    func with(seat: Int) -> PlayniteCoopSeat {
        PlayniteCoopSeat(
            deviceID: deviceID,
...
struct PlayniteCoopSessionState: Sendable, Equatable {
    ...
    /// Device allowed to drive Mac pointer (`PNI1`). Default first video client or seat 1.
    var cursorOwnerDeviceID: String?
    /// Who owns the host-player identity (this Mac, or a companion standing in for the host).
    var hostPlayerDeviceID: String = PlayniteCoopSessionState.localHostDeviceID

    static let maxSeats = PlayniteStreamPorts.maxCoopViewers
    ...
    /// PNG1 joinSeat → current virtual pad.
    ...
    func occupant(seat number: Int) -> PlayniteCoopSeat? {
    ...
    mutating func join(
        ...
        deviceName: String,
        preferredSeat: Int?,
        kind: PlayniteCoopClientKind = .companion,
        playAsHost: Bool = false
    ) -> Result<PlayniteCoopSeat, JoinError> {
        let previousHost = hostPlayerDeviceID
        if playAsHost {
            designateHostPlayer(deviceID: deviceID)
        }
        let result: Result<PlayniteCoopSeat, JoinError>
