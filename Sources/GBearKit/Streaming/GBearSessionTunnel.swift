...
@MainActor
final class GBearSessionTunnelHost {
    static let shared = GBearSessionTunnelHost()

    private let tunnel = GBearSessionTunnel()
    ...
    func startListeningForRelay(coordinatorBaseURL: URL, sessionID: String, deviceID: String) async {
        ...
        do {
            ...
            await tunnel.setHandler { channel, data in
                Task { @MainActor in
                    GBearSessionTunnelHost.shared.handle(channel: channel, data: data)
                }
            }
            active = true
        } catch {
    ...
    private func handle(channel: GBearSessionTunnel.Channel, data: Data) {
        switch channel {
        case .input:
            if let gamepad = GBearGamepadEventFormat.parse(data) {
                Task { await GBearVirtualGamepadManager.shared.apply(gamepad) }
            } else if let keyboard = GBearKeyboardEventFormat.parse(data) {
                GBearKeyboardPlayback.handle(keyboard)
            } else if let touch = GBearInputEventFormat.parse(data) {
                GBearRemoteInputPlayback.handle(touch)
            }
