...
/// Outbound TCP client for length-prefixed `PNA1` PCM (computer guest).
final class PlayniteAudioStreamClient: @unchecked Sendable {
    private var connection: NWConnection?
    private let engine = AVAudioEngine()
    private let player = AVAudioPlayerNode()
    private var started = false
    private var stopping = false
    private let queue = DispatchQueue(label: "PlayniteGuest.audio", qos: .userInitiated)
    ...
    func start(host: String, port: UInt16) {
        ...
        if !started {
            ...
            do {
                ...
                print("[PlayniteGuestAudio] engine start failed: \(error.localizedDescription)")
            }
        }
    ...
    private func handlePacket(_ packet: Data) {
        guard packet.count >= PlayniteAudioFrameFormat.headerSize else { return }
        let magic = packet.withUnsafeBytes { $0.load(as: UInt32.self).littleEndian }
        guard magic == PlayniteStreamPorts.audioMagic else { return }
        ...
        guard payloadLen > 0, packet.count >= PlayniteAudioFrameFormat.headerSize + payloadLen else { return }
        let pcm = packet.subdata(in: PlayniteAudioFrameFormat.headerSize ..< (PlayniteAudioFrameFormat.headerSize + payloadLen))
