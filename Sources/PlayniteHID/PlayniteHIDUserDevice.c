#include "PlayniteHIDUserDevice.h"
...
PlayniteHIDDeviceRef PlayniteHIDDeviceCreate(int seat, const uint8_t *descriptor, size_t descriptorLength) {
    ...
    struct PlayniteHIDDevice *wrapper = calloc(1, sizeof(struct PlayniteHIDDevice));
    if (!wrapper) {
        CFRelease(hid);
        return NULL;
    }
...
void PlayniteHIDDeviceDestroy(PlayniteHIDDeviceRef device) {
    struct PlayniteHIDDevice *wrapper = (struct PlayniteHIDDevice *)device;
    if (!wrapper) return;
    if (wrapper->device) {
