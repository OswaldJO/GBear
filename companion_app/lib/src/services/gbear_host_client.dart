...
/// GBear-native LAN streaming client.
class GBearHostClient {
  GBearHostClient(this._settings);

  final StreamingHostSettings _settings;

  static const protocolVersion = 'gbear-stream/1';
  ...
  Future<Map<String, dynamic>?> fetchStatus() async {
    ...
    try {
      final response = await http.get(_uri('/gbear/v1/status')).timeout(const Duration(seconds: 4));
  ...
  Future<bool> isDevicePaired(String deviceId) async {
    try {
      final response = await http.get(_uri('/gbear/v1/pair/clients')).timeout(const Duration(seconds: 4));
  ...
  Future<String> pairStatus(String deviceId) async {
    try {
      final response = await http
          .get(_uri('/gbear/v1/pair/status', {'deviceId': deviceId}))
  ...
  static bool _macStreamFullyIdle(Map<String, dynamic>? status) {
    ...
    return status['videoStreaming'] != true;
  }

  /// Best-effort stop + idle poll before a new session (Mac listeners stay up).
