...
object GBearCoopPadMappingStore {
    private const val PREFS = "gbear_coop_pad_mappings"
    private const val KEY_PREFIX = "mapping."

    fun load(context: Context, guid: String): GBearCoopPadMapping? {
        val raw = prefs(context).getString(KEY_PREFIX + guid, null) ?: return null
        return runCatching { GBearCoopPadMapping.fromJson(JSONObject(raw)) }.getOrNull()
    }

    fun save(context: Context, mapping: GBearCoopPadMapping) {
    ...
    fun loadOrAutoMap(
        ...
        val mapped = GBearGamepadAutoMapper.autoMap(device, swapFaceButtons)
        save(context, mapped)
        return mapped
    }
    ...
    fun autoMapAndSave(context: Context, swapFaceButtons: Boolean): List<Map<String, Any>> {
        ...
        for (id in InputDevice.getDeviceIds()) {
            ...
            if (!GBearGamepadAutoMapper.isPhysicalGameController(device)) continue
            val mapping = GBearGamepadAutoMapper.autoMap(device, swapFaceButtons)
    ...
    fun all(context: Context): List<Map<String, Any>> {
        ...
        for (key in prefs.all.keys) {
            ...
            val mapping = runCatching { GBearCoopPadMapping.fromJson(JSONObject(raw)) }.getOrNull()
                ?: continue
    ...
    fun applyOverride(
        ...
        val existing = load(context, guid) ?: GBearCoopPadMapping(guid, deviceName, emptyList())
        val binding = if (axis >= 0) {
            GBearCoopPadBinding.axis(logical, axis, invert)
        } else {
            GBearCoopPadBinding.key(logical, keyCode)
        }
